# -*- coding: utf-8 -*-
"""
Created on Fri Apr 10 00:42:59 2020
...
print('Range of x coordinates you can enter  =  -4.5 to 4.5')
print('Range of y coordinates you can enter  = 4.5 to 4.5')
print("Clearance Range = 0-0.5")
print('Wheel radius is calculated to be 76mm/2 -> approximating to 0.5')
start_x, start_y, start_orientation = input("Enter the x and y coordinate of the start and the initial orientation here, with a blank in between them (suggested: -4.5 -4.5 30 : > ").split()
goal_x, goal_y =  input("Enter the x and y coordinate of the goal and the , in the form of blanks (suggested : 4.5 4.5) : > ").split()
start_x = float(start_x)
start_y = float(start_y)
goal_x = float(goal_x)
goal_y = float(goal_y)
x_start = (start_x+5.10)*100
y_start = (start_y+5.10)*100
x_goal = (goal_x+5.10)*100
y_goal = (goal_y+5.10)*100
RPM_L, RPM_R = input("Enter the LEFT WHEEL RPM AND RIGHT WHEEL RPM  : > ").split()
clearance = input("Enter the clearance of the robot with the nearby obstacles : (suggested value  0.1 ): > ")
clearance = float(clearance)*100
step_size = 1
radius = 35
...
print("Checking whether the given points are in obstacle space or boundaries......")
...
cv2.circle(new_canvas,start, radius, (0,0,255),7)
cv2.circle(new_canvas,goal, radius, (0,255,0),7)
cv2.imshow(' < OBSTACLE SPACE MAP > ', new_canvas)
cv2.waitKey(0)
cv2.destroyAllWindows()
    
if start in untraversable_points or goal in untraversable_points or start_x<-4.9 or start_x>4.9 or start_y<-4.9 or start_y>4.9 or goal_x<-4.9 or goal_x>4.9 or goal_y<-4.9 or goal_y>4.9:
    print("###############START or GOAL in Obstacle space. Please check the start/goal nodes and the clearance value")
else:
    time_run = 1
    ...
    # x_start = (int(x_start)+5)*100
    # y_start = (int(y_start)+5)*100
    # x_goal = (int(x_goal)+5)*100
    # y_goal = (int(y_goal)+5)*100
   ...
   #        POINTS FOR DRAWING THE GRAPH
    ...
    def a_star_Algorithm(start, goal):
        ...
        if goal in untraversable_points or start in untraversable_points:
            ...
            while True:
                ...
                quivet_list.append((curr_vert,curr_orient))
                
                # checking if the neighbour is the goal. If goal found reached
                if ((curr_vert[0] - goal[0]) ** 2 + (curr_vert[1] - goal[1]) ** 2 <= (10) ** 2):
                    print('^^^^^^^^^^^^^^')
                    print('^^^^^^^^^^^^^^')
                    print('GOAL REACHED')
                    print('^^^^^^^^^^^^^^')
    ...
    for backtr in backtracked_final:
        ros_backtracked_points.append(((round(((backtr[0]/100)-5.1),2)),(round(((backtr[1]/100)-5.1),2))))
    
    print("Backtracked_path=",ros_backtracked_points)
    # %%
    ...
    for visit_path in new_list_visited:
        ...
        x = int(visit_path[0])
        y = int(visit_path[1])
        cv2.circle(new_canvas_copy_visited, (x, 2040 - y), 7, [0, 255, 255], 5)
    ...
    for path in new_backtracked:
        ...
        x = int(path[0])
        y = int(path[1])
        cv2.circle(new_canvas_copy_visited, (x, 2040 - y), 7, [0, 0, 255], 5)
    ...
    #show visited path
    cv2.imshow('visited', resized_visited)
