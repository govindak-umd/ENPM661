...
x_start, y_start, start_orientation = -430.0,-380.0,0.0
x_goal, y_goal = 445.0,300.0
RPM_L, RPM_R = 20,30
radius = 35.0
clearance = 20.0
step_size = 1.0
    ...
    try:
        ...
        for index in range(len(translated_ros_backtracked_points)-1):
            ...
            while (x - goal_x)**2 + (y - goal_y)**2 > 0.3:
                # print('WHIL - ED AGAIN')
                print 'radar value :: ', abs(math.degrees(theta)-abs(math.degrees(atan2(goal_y - round(y,2) ,goal_x - round(x,2)))))
                if abs(math.degrees(theta)-abs(math.degrees(atan2(goal_y - round(y,2) ,goal_x - round(x,2)))))<5:
                    ...
                    if math.degrees(theta)-math.degrees(atan2(goal_y - round(y,2) ,goal_x - round(x,2))) > 0:
                        ...
                        h.append((round(x,2),round(y,2)))
                    # else:
                    print('Moving forward along path ANYWAY!!!!! ')
                    GoForward()
                    h.append((round(x,2),round(y,2)))
