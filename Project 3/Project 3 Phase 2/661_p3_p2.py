# -*- coding: utf-8 -*-
"""
Created on Sat Mar 21 18:42:24 2020
...
def generateGraph(point,size_x,size_y,orientation): #remember that this size_x and size_y are the sizes of the matrix, so not the end coordinates
...
def a_star_Algorithm(start,goal):
    ...
    if goal in list_of_obstacle_points or start in list_of_obstacle_points:
        ...
        priority_queue = [(0,start,start_orientation)]
        ...
        while len(priority_queue)>0 and check!=[]:
            ...
            if checkObstaclespace(curr_vert)==True:
                graph = generateGraph(curr_vert,301,201,orientation)
    ...
    #returning all_distance, backtracked dictionary, rounded neighbours
...
def generateChilds(backtrack):
    ...
    for parent in backtrack:
        child = generateGraph(parent,301,201,orientation)
